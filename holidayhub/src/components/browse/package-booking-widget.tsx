...
import { Calendar, Users, ShieldCheck, Minus, Plus } from "lucide-react";
...
export function PackageBookingWidget({
  ...
  const checkoutHref = `/checkout?type=package&id=${listingId}&batch=${slotId}&qty=${qty}`;
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-lg">
      ...
      {/* Batch date selector */}
      <div className="mb-4">
        <label className="mb-2 block text-sm font-medium text-gray-700">Choose departure</label>
        {batches.length === 0 ? (
          <p className="rounded-xl bg-gray-50 p-3 text-sm text-gray-500">
            No open batches right now. New weekend departures are added weekly.
          </p>
        ) : (
          <select
      ...
      {/* Qty */}
      <div className="mb-4">
        <label className="mb-2 block text-sm font-medium text-gray-700">Travelers</label>
